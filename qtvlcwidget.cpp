...
public:
    ...
    QOpenGLFramebufferObject *getVideoFrame()
    {
        ...
        if (m_updated) {
            std::swap(m_idx_swap1, m_idx_display);
            m_updated = false;
        }
                                     ...
                                     libvlc_video_output_cfg_t *render_cfg)
    {
        ...
        that->mBuffers[3] = new QOpenGLFramebufferObject(cfg->width, cfg->height);
    ...
    static void cleanup(void* data)
    {
        ...
        delete that->mBuffers[3];
        that->mBuffers[3] = NULL;
    }
    ...
    static void swap(void* data)
    {
        ...
        std::swap(that->m_idx_swap2, that->m_idx_swap1);
        std::swap(that->m_idx_swap2, that->m_idx_render);
...
private:
    ...
    unsigned m_height = 0;
    QMutex m_text_lock;
    QOpenGLFramebufferObject *mBuffers[4];
...
bool QtVLCWidget::playMedia(const char* pathFile)
{

    //m_media = libvlc_media_new_location (m_vlc, url);
    m_media = libvlc_media_new_path(m_vlc, pathFile);
    if (m_media == nullptr) {
        fprintf(stderr, "unable to create media %s", pathFile);
        return false;
    }
